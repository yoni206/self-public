...
def gen_at_most_one_int(vars, solver):
    if len(vars) <= 1:
      return solver.mkTrue()
    else:
      one = solver.mkInteger(1)
      sum = solver.mkTerm(Kind.ADD, *vars)
      leq = solver.mkTerm(Kind.LEQ, sum, one)
      return leq

def gen_at_most_one_bool(vars, solver):
    conjuncts = []
...
variables = {}
for e in edges:
    variables[e] = solver.mkConst(int_sort, str(e))
for d in donors:
...
for recipient in recipients_to_edges:
    ...
    at_most_one = gen_at_most_one_int(edges_vars, solver)
...
for donor in donors_to_edges:
    ...
    some_edge_selected = solver.mkTerm(Kind.OR, *variables_for_edges)
